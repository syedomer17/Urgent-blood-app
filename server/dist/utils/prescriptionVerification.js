...
const verifyPrescriptionDocument = async (fileBuffer, mimeType) => {
    try {
            ...
            contents: [
                {
                    parts: [
                        {
                            text: PRESCRIPTION_VERIFICATION_PROMPT,
                        },
                        {
                            inlineData: {
                                mimeType,
        ...
        const response = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash:generateContent?key=${env_1.config.gemini.apiKey}`, {
            method: 'POST',
            headers: {
