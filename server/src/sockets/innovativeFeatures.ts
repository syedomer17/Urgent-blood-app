...
import { DonationHistory } from '../modules/donations/donation.model';
...
export class InnovativeFeatures {
    /**
    ...
    static async generateDonorLeaderboard() {
        try {
            ...
            socketManager.broadcast('leaderboard_updated', {
    ...
    static async triggerEmergencyBroadcast(data: {
        ...
        try {
            ...
            socketManager.broadcast('emergency_broadcast', {
                bloodGroup: data.bloodGroup,
            ...
            logger.info(`🚨 Emergency broadcast sent to ${donors.length} donors`);
        } catch (error) {
    ...
    static async generateLiveHeatmap() {
        try {
            ...
            socketManager.broadcast('live_heatmap_update', {
                heatmap: heatmapData,
                totalDonors: heatmapData.reduce((sum: number, item: any) => sum + item.count, 0),
                timestamp: new Date(),
            });
     ...
     * - Communication
     */
    static async updateTrustRating(
        donorId: string,
        rating: number,
        review: string,
        requestId: string
        ...
        try {
            ...
            // Notify donor
            socketManager.emitToUser(donorId, 'trust_rating_updated', {
                newRating: newRating.toFixed(1),
                totalReviews: existingCount + 1,
                message: 'Your trust rating has been updated!',
            });

            logger.info(`✅ Trust rating updated for donor ${donorId}: ${newRating.toFixed(1)}`);
        } catch (error) {
    ...
    static async generateLiveAnalytics() {
        try {
            ...
            const recentDonations = await DonationHistory.aggregate([
                {
                    $match: {
                        createdAt: {
            ...
            socketManager.broadcast('live_analytics_update', analytics);
            return analytics;
        } catch (error) {
     ...
     */
    static async findPerfectMatches(requestId: string) {
        try {
            const request = await BloodRequest.findById(requestId);
            if (!request || !request.location?.coordinates || request.location.coordinates.length !== 2) return;

            const coordinates = request.location.coordinates as [number, number];
            ...
            logger.error('Error finding perfect matches:', error);
        }
    }
    ...
    static async sendDirectMessage(
        ...
        try {
            ...
            socketManager.emitToUser(recipientId, 'new_direct_message', {
                from: {
                    id: senderId,
                    name: sender?.name,
     ...
     */
    static async checkDonorAchievements(donorId: string) {
        try {
            const donor = await User.findById(donorId);
            ...
            if (donations === 1) achievements.push('first_donation');
            if (donations === 5) achievements.push('dedication_5');
            if (donations === 10) achievements.push('dedication_10');
            if (donations === 25) achievements.push('platinum_donor');
            if (donor?.trustRating! >= 4.8) achievements.push('trusted_hero');
            if (donor?.trustRating! >= 4.5) achievements.push('excellent_donor');

            if (achievements.length > 0) {
                socketManager.emitToUser(donorId, 'achievement_unlocked', {
                    achievements,
     ...
     */
    static async notifyProximityAlert(userId: string, coordinates: number[]) {
        try {
            ...
            if (nearbyRequests.length > 0) {
                socketManager.emitToUser(userId, 'proximity_alert', {
     ...
     */
    static async setNotificationPreferences(userId: string, preferences: any) {
        try {
            ...
            socketManager.emitToUser(userId, 'preferences_saved', {
    ...
    // Helper
    static getDonorBadge(rank: number) {
        if (rank === 1) return '🥇';
        if (rank === 2) return '🥈';
        if (rank === 3) return '🥉';
        if (rank <= 10) return '⭐';
